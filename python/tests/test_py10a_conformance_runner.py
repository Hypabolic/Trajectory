...
def test_claim_writer_progressive_pi_normalize() -> None:
    """Claim-writer floor from PY-10a: pi remains claimed; letta/canonical outputs.

    Exact progressive membership expands via later PY-10b claim-writers
    (e.g. test_py10b_sources_claude_codex). This pins the PY-10a floor only.
    """
    ...
    assert data["sources"][0] == "pi"
    assert "letta-trajectory-v1" in data["outputs"]
    assert "letta-canonical-v1" in data["outputs"]
    # Progressive capabilities covered by filtered normalize-letta/canonical.
