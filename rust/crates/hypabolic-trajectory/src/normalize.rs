...
/// Native Cursor Agent transcript JSONL source adapter.
#[derive(Debug, Default, Clone, Copy)]
...
/// Normalizes Cursor Agent `agent-transcripts` JSONL.
pub fn normalize_cursor(request: NormalizeRequest<'_>) -> Result<Trajectory, TrajectoryError> {
    let config = resolve_config(request)?;
