...
implemented as core stream apply APIs. Core stream AHP capabilities
(`stream-ahp-snapshot`, `stream-ahp-action-log`) are claimed on the
compatibility manifest. See [streaming.md](../streaming.md) and
[ahp-source-spec](../../../docs/ahp-source-spec.md). Export-directory listing
and a first-class live WebSocket host remain caller-owned.
