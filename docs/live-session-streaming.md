...
Status: **locked design**; wire contracts landed (LS-01); core stream state + snapshot + append (LS-03–LS-05) landed; AHP snapshot + action-log (LS-06–LS-07) in-tree on tip; **LS-08 full stream matrix gate green** (shared goldens + oracle on all four runtimes); **LS-09 optional file I/O packages** and **LS-10 optional AHP clients** in-tree (poll/follow helpers; transport-only clients + fake-host CI; no capability advertising yet); sample CLIs and capability advertising remain LS-11–LS-12  
