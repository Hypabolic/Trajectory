...
public sealed class ClaudeCodeParityTests
{
    ...
    public void SanitizedVersionFamilyFixturesMatchPinnedUpstreamTrajectory(
        string fixtureName)
    {
        var input = Fixture($"claude-code/{fixtureName}/input.jsonl");
        var expected = JsonNode.Parse(
            Fixture($"claude-code/{fixtureName}/expected.letta.json"))!;
    ...
    {
        var input = Fixture("claude-code/tool-call/input.jsonl");
        var expected = JsonNode.Parse(
            Fixture("claude-code/tool-call/expected.canonical.json"))!;
    ...
    {
        var input = Fixture("claude-code/mixed-version/input.jsonl");
        var trajectory = TrajectoryConverter.NormalizeToIR(
            TrajectorySource.ClaudeCode,
            input);
